...
const wordInfo = {
  ...
  while (!won) { // Will loop over block until condition isn't true
    history.count += 1;
    console.log('Try turns: ' + history.count);
  ...
  function commonLetter(guess, wordInfo) {
  ...
  function thinkAbout( wordInfo ) {
    ...
    // EDIT BELOW THIS
    wordInfo.range = [0, wordInfo.allWords.length - 1];
    // EDIT ABOVE THIS
  }
  ...
  function pickGuess( wordInfo, history ) {
    ...
    // EDIT BELOW THIS
    let mid = Math.floor((wordInfo.range[0] + wordInfo.range[1]) / 2);
    return wordInfo.allWords[mid];
    // EDIT ABOVE THIS
  }
  ...
  function compareLetters( guess, wordInfo ) {
    ...
    result.similar = commonLetter(guess, wordInfo);
  
    if (guess < wordInfo.word) {
      wordInfo.range[0] = wordInfo.allWords.indexOf(guess) + 1;
    } else if (guess > wordInfo.word) {
      wordInfo.range[1] = wordInfo.allWords.indexOf(guess) - 1;
    } else {
      result.won = true;
    }
