...
* You will receive lessons and material by copying updates that are made to the github repo to your own local repo (a 'pull')
* You will do any of your coding and work on your machine, saving to your local repository as needed/desired
...
* a *project* is any collection of code that is tracked in one or more git repos (see below) - this is not a technical term of git, but a common usage
* a git *repo* is any directory tree that git has been told to track (which is done with `git init`)
    * You can have many git repos on your machine or elsewhere, of the same project or different ones
...
* Your changes are now recorded to the current branch (_some-new-branch_) in the local repo.  If you have more changes to do for a different reason (and different commit message), now you can do them.  Once all your changes are done, options include:
    * `git checkout master` - just like we started, this takes us back to the master branch.  What follows can be done with any branch, but your primary branch is the most common recipient, and _master_ is the name commonly used for a primary branch
    * `git merge some-new-branch` - this pulls the current contents of _some-new-branch_ and *merges* it into the current branch. 
